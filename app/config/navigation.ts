...
export const allRoutes: NavigationItem[] = [
  ...
  {
    title: "Quizzes",
    href: "/console/quizzes/assign/[id]",
    icon: "i-heroicons-clipboard-document-list",
    access: ["ADMIN"],
    showInSidebar: false,
  },
...
export const navSections: NavigationSection[] = [
  {
    title: "Core",
    access: ["ADMIN", "MODERATOR", "STUDENT"],
    items: allRoutes.filter(
      (route) =>
        route.showInSidebar &&
        ["dashboard", "join-requests", "resources-hub"].some((path) =>
          route.href.includes(path)
        )
    ),
  },
  {
    ...
    items: allRoutes.filter(
      (route) =>
        route.showInSidebar &&
        ["classes", "attendance", "lessons", "quizzes"].some((path) =>
          route.href.includes(path)
        )
    ),
  },
  {
    ...
    items: allRoutes.filter(
      (route) =>
        route.showInSidebar &&
        ["students", "moderators", "fees", "products"].some((path) =>
          route.href.includes(path)
        )
    ),
  },
